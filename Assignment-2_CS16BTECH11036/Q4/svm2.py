...
if len(sys.argv) != 2:
    print "Enter number of rows to include" 
else :
    ...
    rows_inc = int(sys.argv[1])
    train_data = train_data[:rows_inc]
    train_labels = train_labels[:rows_inc]
    test_data = test_data[:rows_inc]
    test_labels = test_labels[:rows_inc]
